...
Parallel Windows Update deployment tool for rolling out updates to remote computers across multiple sites. Starts update jobs simultaneously, monitors progress with smart completion detection, collects results, and generates an interactive HTML report.

## Features

- **Parallel deployment** — Updates all target machines simultaneously (PowerShell 7 `ForEach-Object -Parallel`)
- **Smart monitoring** — Detects completion via marker files, scheduled task state, WU job status, and log stability heuristics
...
- **Interactive HTML report** — Dashboard with stats, failure alerts, per-computer expandable cards, search, filtering, and sort by status
- **Re-run CSV** — Automatically generates a CSV of failed/unreachable machines for easy re-execution
...
flowchart TD
    ...
    Creds --> CSV[Select CSV File]
    CSV --> Validate{Validate CSV\nName + IP columns}
    Validate -->|Invalid| Exit([Exit])
    Validate -->|Valid| P1
    ...
    subgraph P3 [Phase 3: Collect Results]
        direction TB
        P3a[Open PSSession\nper computer] --> P3b[Copy updatelog.csv\nfrom remote]
        P3b --> P3c[Parse update results\ntext + numeric codes]
        P3c --> P3d[Build per-computer\nsummary]
    ...
    subgraph P4 [Phase 4: Generate Report]
        direction TB
        P4a[Calculate statistics] --> P4b[Build interactive\nHTML report]
...
Session_20250315_143022/
  ...
  all_updates.csv              # Every update across all machines
...
- Stats dashboard with total computers, updates installed/failed, reboot counts
- Failure alert banner with clickable links to affected computers
- Per-computer expandable cards showing each update with KB, title, and result
- Search and filter by computer name or update title
- Sort computers by status (failures first)
- Download re-run CSV directly from the report
